...
- `scripts/run_all.py` — 主流程：跑五軌、併欄、寫報告、推播。
  ...
  📌**整批抓取失敗偵測**(`_data_health`，2026-08)：今日 price 筆數 < 前一交易日 70% 就判定某市場
  整批沒抓到，回一句警示掛在 HTML 報告最頂端(紅底)、MD 第一段、**Telegram 推播第一行**，並印在 console。
  **實際踩過**：8/27 首抓 TWSE 整批失敗(log 只印 `twse:—`)，DB 只進上櫃 887 檔(正常 1972)，
  但五軌照跑、報告照出、推播照成功——上市股全部從候選消失而報告看不出異常。門檻用「跟前一交易日比」
  而非固定值(免受市場自然增減影響)；實測完整資料不誤報。
  ⚠️同類問題**月營收 OpenAPI 也有**(8/28 只回 1/3 資料→T12 從 236 掉到 76，重跑即恢復)，
  但那條**還沒加偵測**。
  📌**picks 標籤用『資料日』而非執行日**(`run_all` `data_day`，2026-08 修)：原本 `pt.save(today)` 用
  `date.today()`，但執行日與資料日常不同(盤前跑、容器 UTC vs 台北、補跑舊日)。實際踩到：台北凌晨
  (UTC 還是前一天)先後跑了 8/26 與 8/27 兩份盤後，兩次都標成同一執行日，**後者把前者整份蓋掉**
  (save 會先清當日再重寫)→ 黑名單明細沒了、追蹤區直接斷。改用 DB price 最新交易日當標籤，
  `followthrough`/`save`/`snipe_ohlc` 三處一致；報告檔名維持執行日(不動既有慣例)。
- `src/report_html.py` — HTML 輸出＋`COLUMN_LABELS`（英文欄名→中文顯示名）＋`rename_cn`。
  ...
  📌**表頭可點排序**(`_SORT_JS`/`sortT`)：任一欄點一下降序、再點升序，空值一律沉底；
  數字自動辨識(去掉 % , + 張 倍 億 元 後 parseFloat)。純前端視圖，**不動各軌本身的排序**
  ——那是策略的一部分(T16 依 RS 排＝回測驗證過的)。使用者想「依風險低到高看」就點 `停損%`。
  📌**區塊可收合**(`_sec`/`_sec_wrap`/`_secctrl`/`_SEC_JS`)：每軌標題可點收合(▾/▸)，
  頂端「全部收合／全部展開」兩顆鈕，localStorage 記住(下次開報告不看的軌仍收著)。
  key 用 `b['key'] or b['bt'] or sec{i}`——持股區塊標題含金額會變故給固定 `key:"持股"`；
  追蹤區走 `_sec_wrap`(拆既成的 `<h2>…</h2>` 再包)，不用改那兩個函式本身。純視圖、MD 無對應。
...
- `src/screeners/daytrade_snipe.py` — **第6軌 隔日沖鎖碼候選(實驗)**：DB 抓今日漲停/大漲(≥9%)→取成交額前N檔→
  ...
  📌**兩份黑名單都帶『各買幾張』**(2026-08)：名字後標 `+N張`、**依張數排序**(篩選看隔日沖率是不是慣犯，
  排序看量體——率再高只買20張威脅不大)；`黑名單買張`＝合計、`黑名單明細`＝逐分點買量(最多6個、格式 `名字+張數`、
  頓號分隔)。明細**存進 picks.csv** → 隔日追蹤區才能逐點算「這個人最後倒了幾張」(見 `picks_tracker.snipe_ohlc`)。
...
- `src/walkforward.py` — walk-forward 共用純計算(`curve_metrics` 權益曲線指標／`bh_curve` 買入持有基準／
  `make_folds` expanding-train 折切25/50/75%)。T16/T12 OOS 共用，避免兩份漂移。⚠️抽出時已驗證 T16 報告逐字不變。
...
- 報告：`reports/signal_compare.md`、`reports/t16_tune.md`、`reports/portfolio_backtest.md`、`reports/oos_validation.md`、`reports/longterm_backtest.md`、`reports/t16_oos.md`、`reports/t12_oos.md`、`reports/daytrade_signal_backtest.md`、`reports/snipe_signal_backtest.md`。
...
python -m scripts.run_t12_oos                          # T12 嚴格樣本外 walk-forward（同框架，共用 src/walkforward）
python -m scripts.run_t11q_oos                         # T11Q 潛伏吸籌 樣本外（結論：無 edge、未上線）
```
...
- **T12 月營收動能＝in-sample 看似有 edge，但🔬嚴格樣本外(walk-forward)失效**（`run_t12_oos`，2026-08）：
  連續 OOS **CAGR 僅 +7.8%／夏普 0.41／回撤 −49%**，**大輸同期買入持有(+26.5%／1.07)**；
  「看整段挑最好」+34.5%／1.25 是上界 → **資料窺探灌水約 4 倍**。逐折高度分散(F1 +11%、**F2 −36%**、F3 +76%)、
  參數跳動(regime 有無、YoY門檻都換)。結論：**T12 別當進出場策略**，跟 T11/長期價值一樣降級為選股情報。
  以下為原 in-sample 數字（保留對照，已知高估）：（199 檔月營收面板實測）：持5檔/20日 CAGR **+21.5%**、
...
- **T16 排序方式已測（`run_t16_rank_compare`，2026-08）：維持 RS，別改成風險排序**。
  使用者問「能不能改成風險低→高排序」→ 實測（持10檔/20日/無停損，離線200檔面板）：
  RS原版 **+31.8%/夏普0.99**；**低波動優先只剩 +2.4%/0.25**（回撤確實降到 −23.6%，但報酬幾乎歸零
  ——T16 的 edge 來源就是動能，照風險排等於把動能濾掉）；風險調整(RS÷波動率) +11.6%/0.55，
  +regime 才追到 +24.4%/**夏普1.02**(唯一略勝原版之處，但 CAGR 少 7pp、回撤沒改善)。
  → **預設維持 RS**；想控風險改用「HTML 點 `停損%` 表頭排序」(純視圖、不動策略)。
  `compute_t16_entries(rank=)` 保留 rs/lowvol/riskadj 三模式供日後再測。
  ⚠️此配置下 **regime 對 T16(持10檔) 反而變差**(+31.8→+24.5、回撤 −36.9→−40.3)，
  與早期「順 regime」的結論不一致(參數不同)；要用 regime 前該重測，別照抄舊結論。
- **T11Q 潛伏吸籌＝樣本外沒 edge，不上線**（`run_t11q_oos`，2026-09）：使用者想找「法人持續
  吸籌但還沒漲、類似整理期」的股票 → 把 T11 的價格條件**反過來**（吸籌+橫盤+量縮+不在高檔+貼均線）。
  in-sample 初掃很漂亮（持40~60日 CAGR 20~25%／夏普 1.2~1.37，**優於 T16 的 0.99**），
  但 walk-forward OOS **CAGR +21.6%／夏普 1.04／回撤 −35.1%，輸同期買入持有(+26.5%／1.07)**。
  逐折極度分散：F1 +2.8%、**F2 −19.6%(夏普 −1.04)**、F3 +117.4%——整條曲線靠 F3 撐，
  跟 T16 的 F3 +195% 同樣是強多頭×倖存者偏誤。參數也跳(holding 20→60→60、regime 有無都換)。
  ✅**有價值的副產品**：持有期是決定性旋鈕——**持20日夏普僅 0.48~0.62、持40~60日才 1.2~1.37**，
  潛伏股需要時間發動，用短持有期測會「還沒發動就出場」。日後若再試這類策略，別用 20 日。
  ⚠️「千張大戶同步加碼」那條**沒進回測**(面板只有價量+法人，大戶是週頻另存 DB)——
  未經驗證，別假設加了就會變好。`compute_t11q_entries` 留在引擎裡供日後再測。
- **實用結論**：波段用 **T16+分散**（攻）；長期用 **買入持有+regime**（守，勝過價值選股）；
  **T12 樣本外失效→與 T11、長期價值一併降級為『選股情報』，別當策略**。
...
1. **全軌命中追蹤＋累積戰績表**（推薦）：`picks_tracker` 追 T11/T16/當沖/**隔日沖鎖碼**(已加,2026-08)，
   ...
   ＋**解釋漲跌原因欄(2026-08)**：`預估賣壓%`(pick 當日存下的預測) vs `實際賣壓%`(今日昨買今賣對沖量÷今量)＝預測兌現否、
   `今主力淨額`(今日主力續買撐盤/在倒)、`高檔回落%`(今收vs今高)、`振幅%`/`量能倍數`/`當沖比%`(對殺熱度)。
   實測規律清楚：今主力淨額正→多收在高檔(高檔回落≈0)、負→開高走低(如南電−2806→盤中−6.2%、環球晶+1025→+5.7%)。
   ⚠️這些是**同日解釋變數(事後看原因)**，非隔日預測訊號。分點欄需 Sponsor。
   💣**逐分點倒貨對照(2026-08)**：`黑名單買張`(昨合計買)→`黑名單賣張`(今合計倒)、`倒貨%`＝賣÷買、
   `黑名單逐點`＝「分點 買X→賣Y(Z%)」逐一列，今日反手續買標『今再買』。解析昨天存下的 `黑名單明細`
   (`_parse_bl`，regex `名字+張數`)＋今日 `broker_signal._branch_net`(已是張、不再除1000)。
   ⚠️**倒貨%可>100%**(賣得比昨天買的多＝手上有更早的貨或反手加空)，不是 bug。
   實測(8/25→8/26)：鼎元凱基台北買1625→賣950(58%)但富邦反手續買+5176→盤中仍+11%；台虹倒貨僅5.4%→+6.1%；
   華星光凱基站前買488→賣468(96%)→盤中−2.1%。**無分點(無Sponsor)時整組留白，不謊報「沒賣」**。
...
3. ✅**T12 嚴格樣本外已完成**（2026-08，`run_t12_oos`）：結論＝**OOS 失效**(+7.8%/0.41 vs 買入持有 +26.5%/1.07)，T12 降級為情報。回測嚴謹缺口已補完。
- 更大工/邊際遞減：盤中即時(Shioaji VWAP/五檔/內外盤，另一個專案)、價值軌納配息年數/EPS、上櫃估值。
- **使用者當前態度＝先每天跑 `1_盤後選股.bat` 觀察，讓 picks_tracker／分點檔案累積真實樣本再決定。**
- 🚫**明確不做**（使用者 2026-08 拍板）：盤前自動化推播(美股/ADR/VIX/事件日曆)——「當沒這回事」；
  爬蟲類(董監持股、法說會、新聞題材)——維護成本高、易靜默失效。
- 剩餘可做：①全軌命中追蹤累積戰績表(T12/長期尚未納入 picks_tracker) ②T16 驗證贏法變「今日建議籃子」。
...
### 📌 2026-08 投資框架補洞 + 分點檔案 session 收尾（**最新，下次接手先看這裡**）
本 session 全部已 push、153 測試全綠。主軸＝依使用者提供的三策略投資框架盤點缺口並補齊，
再把「隔日沖分點判斷」從單檔小樣本升級成跨股票建檔。

**A. 報告易用性**：HTML 收盤價上限篩選(`_pxctrl`)、代號→Goodinfo K線圖外連(`_klink`)、
   **精簡模式**(`_CORE_COLS`，45~51欄→22~25欄)。
**B. 個股深掘擴成完整病歷表**：技術面(均線疊圖+綜合定調)／基本面(月營收/獲利/估值/配息)／
   財務體質三表(負債比/流動比/BPS/營運CF/含金量/自由現金流/**真實ROE**)／同業比較／財報紅旗／三率三升。
**C. 風控補洞**(五軌+第6軌全套)：處置股警示(`disposal`)、ATR停損目標價(`risk.levels`)、
   除權息預告(`exdividend`)、外資持股%/市值/周轉率(`shareholding`)、半年線120MA。
**D. 分點升級**：`broker_profile` 跨股票建檔→全市場黑名單/本檔黑名單分兩欄＋前瞻預估賣壓；
   計數器持久化進 DB/CSV(跨機器累積)；控制台「📇 分點黑名單」鈕。
**E. 追蹤區補解釋欄**：預估賣壓% vs 實際賣壓%、今主力淨額、高檔回落%、振幅/量能/當沖比。
**F. 修的 bug**：`broker_client` 漏 `load_dotenv`(Sponsor 分點誤判不可用)、
   分點檔案把快取「缺日」誤當隔日(已加相鄰交易日檢查，回測數字因此下修)。
**G. 法人/資券堆疊欄改今/昨/10/20**(加昨日、拿掉60日)。
**H. T12 嚴格樣本外(`run_t12_oos`)＝最後一個回測缺口，結論失效**：OOS CAGR +7.8%/夏普0.41，
   **大輸買入持有(+26.5%/1.07)**；「看整段挑最好」+34.5% 是灌水 4 倍 → T12 降級為選股情報。
   共用計算抽到 `src/walkforward.py`(T16/T12 同一份，抽出後重跑 T16 報告逐字不變才提交)。
   **現況：T16 是唯一通過嚴格樣本外的軌；T11/T12/長期皆無 edge。**
**I. 各軌回測驗證徽章**(`report_html.BACKTEST_VERDICTS`)：報告每軌標題旁直接標
   ✅/❌/⚠️＋一行數字，避免看到清單就以為都能賺。改結論只改這個 dict。
**J. 輸出改選用**：預設**只出 HTML**；`--md`/`--csv` 才另出，控制台「📊 盤後選股」下方兩個勾選框。
   ⚠️CSV 沒輸出時，「完整清單見 CSV」註記自動改成「需勾選後重跑」(不開空頭支票)。

⚠️**下次接手注意**：
- 我的測試跑 run_all 會改寫 `data/history/picks.csv`＋各資料 CSV → **commit 前務必 `git checkout` 還原**
  (資料以使用者本機實跑為準)；`datastore.dump()` 不帶 `--keep-days 260` 會撐大資料 CSV(破壞滾動窗)。
- `broker_profile.csv` 會隨時間長大(現 ~255KB，`stocks` JSON 佔大宗)；若哪天過大，可考慮截斷 stocks 清單。
- 使用者態度：**先實戰觀察**，累積 picks/分點樣本後再談新功能。
...
### 📌 2026-09 實戰觀察期 session 收尾（**最新，下次接手先看這裡**）
使用者開始每天實跑並提需求，本 session 全部已 push、177 測試全綠。做了：

1. **黑名單逐點倒貨**（第6軌＋追蹤區）：兩份黑名單帶『各買幾張』(依張數排序)、`黑名單明細`
   存進 picks.csv，隔日追蹤算出 `黑名單買張→黑名單賣張／倒貨%／黑名單逐點`。
   實戰規律頗一致：**倒貨%低＋有人反手續買→續攻；倒光且無人接→走弱**(但有反例，是傾向非規則)。
2. **報告易用性**：表頭可點排序(`_SORT_JS`)、**區塊可收合**(`_sec`/`_SEC_JS`，使用者嫌滑太久)。
3. **修的 bug**：picks 標籤用資料日(見上，追蹤曾被蓋掉)、`_data_health` 整批抓取失敗偵測(見上)。
4. **千張大戶改盤後自動更新**(`_update_holders`)＋歷史回補到 109 週(2024-07~2026-09)。
5. **T11Q 潛伏吸籌回測**：使用者想要「法人一直買但還沒漲」的軌 → 樣本外**輸買入持有，未上線**。
   副產品：持有期是決定性旋鈕(持20日夏普 0.5、持40~60日 1.2~1.37)。
6. **T16 排序方式實測**：維持 RS，別改成風險排序(低波動優先報酬幾乎歸零)。
7. **排程折騰一輪**：self-bind → fresh session → **改回 self-bind**(見下節，fresh 不能用)。

⚠️**下次接手注意**：
- 使用者主要用手機看 **Telegram 推播**；HTML 報告靠 SendUserFile 傳。`reports/screener/` 不進 git。
- 排程若延遲沒跑，使用者會直接說「我沒收到報告」→ 先查 `git log` 有沒有當日 data commit、
  再查 `list_triggers` 的 `last_run`，別急著重跑。
- **月營收 OpenAPI 間歇性只回部分資料**（T12 檔數異常偏低就是這個），重跑即可，但尚未做偵測。
