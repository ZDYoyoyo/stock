...
def main():
    ...
    notesnipe = ("📅 " + (f"{_snipe_asof}。" if _snipe_asof else "")
                 ...
                 "僅隔日沖賣壓%極端(≥20%)才弱預告次日偏空。僅圈定舞台、非多空訊號。"
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        _section(f, "🎯 隔日沖鎖碼候選｜漲停/大漲 + 主力/隔日沖大戶鎖碼（明日對殺 arena，非即時訊號）", dfsnipe,
                 ["stock_id", "name", "market", "產業", "close", "漲跌%", "成交額億", "今日量張", "量能倍數", "當沖比率%",
                  "均線排列", "季線年線", "20MA乖離%", "52週位置%", "外資今日", "投信今日", "券資比%", "借券賣出餘額", "借券增減",
                  "昨主力淨額", "今主力淨額", "隔日沖賣壓%", "籌碼訊號", "隔日沖鎖碼"],
                 note=notesnipe)
    ...
    blocks = [
        ...
        {"title": "🟡 波段｜T16 抗跌強勢", "df": df16, "note": note16, "n": 15,
         ...
         "signed": ["今日漲跌%", "20MA乖離%","return_%", "vs_market_%", "外資今日", "投信今日", "自營今日", "融資今日", "融券今日", "外資", "投信", "自營", "融資增減", "融券增減", "外資連", "投信連", "自營連", "主導度%", "融資佔量%", "融券佔量%", "借券增減", "主力淨額", "籌碼訊號"],
        ...
        {"title": "🎯 隔日沖鎖碼候選｜漲停/大漲 + 主力/隔日沖大戶鎖碼（明日對殺 arena，非即時訊號）",
         ...
         "cols": ["stock_id", "name", "market", "產業", "close", "漲跌%", "成交額億", "今日量張", "量能倍數", "當沖比率%",
                  "均線排列", "季線年線", "20MA乖離%", "52週位置%", "外資今日", "投信今日", "券資比%", "借券賣出餘額", "借券增減",
                  "昨主力淨額", "今主力淨額", "隔日沖賣壓%", "籌碼訊號", "隔日沖鎖碼"],
         "signed": ["漲跌%", "20MA乖離%", "外資今日", "投信今日", "借券增減", "昨主力淨額", "今主力淨額", "籌碼訊號"]},
    ]
    if not pf_view.empty:
        blocks.append({
