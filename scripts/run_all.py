...
def main():
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        _section(f, "🎯 隔日沖鎖碼候選｜漲停/大漲 + 主力/隔日沖大戶鎖碼（明日對殺 arena，非即時訊號）", dfsnipe,
                 ["stock_id", "name", "market", "產業", "close", "漲跌%", "成交額億", "當沖比率%", "昨主力淨額", "今主力淨額", "隔日沖賣壓%", "隔日沖鎖碼"],
                 note=notesnipe)
    ...
    blocks = [
        ...
        {"title": "🎯 隔日沖鎖碼候選｜漲停/大漲 + 主力/隔日沖大戶鎖碼（明日對殺 arena，非即時訊號）",
         ...
         "cols": ["stock_id", "name", "market", "產業", "close", "漲跌%", "成交額億", "當沖比率%", "昨主力淨額", "今主力淨額", "隔日沖賣壓%", "隔日沖鎖碼"],
         "signed": ["漲跌%", "昨主力淨額", "今主力淨額"]},
    ]
    if not pf_view.empty:
        blocks.append({
